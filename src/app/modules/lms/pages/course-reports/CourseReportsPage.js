...
export function CourseReportsPage({ match }) {
  ...
  console.log("cıngırak", match);

  let pushToUrl = (coid) => {
    console.log("zzzzzzzz", coid);
    history.push(`/lms/course-reports/course/${match.params.cid}/${coid}`);
  };
  ...
  React.useEffect(() => {
    ...
    if (match.params !== undefined && match.params !== null) {
      axios
        .get(`/course-usage/course/${match.params.cid}/${match.params.coid}`)
        .then((res) => {
  ...
  const handleFilterByTime = () =>{
    ...
    {
      axios
      .post(`/course-usage/course/filter/${match.params.cid}/${match.params.coid}`,{begined,finised})
      .then((res) => {
        console.log(res.data, match.params.cid);
        setUsers(res.data);
      });
  ...
  return (
    <Card>
      <CardHeader title="Kurs Raporları" />
      <CardBody>
        ...
        <table class="table">
          <thead>
            <tr>
              ...
              <th scope="col">Geçirilen Saat</th>
            </tr>
          </thead>
          <tbody>
            {users.map((user) => {
              return (
                <tr>
                  ...
                  <td>{user.CourseUsageTimeAsHour}</td>
                </tr>
              );
            })}
          </tbody>
