...
export function UserPage({ history }) {
  const usersUIEvents = {
    newUserButtonClick: (cid) => {
      history.push(`/lms/reports/new/company/${cid}`);
    },
    openEditUserDialog: (id, cid) => {
      history.push(`/lms/reports/${id}/edit/company/${cid}`);
    },
    openDetailUsersDialog: (id, cid) => {
      history.push(`/lms/reports/${id}/detail/company/${cid}`);
    },
    openDeleteUserDialog: (id, cid) => {
      history.push(`/lms/reports/${id}/delete/company/${cid}`);
    },
    openDeleteUsersDialog: (cid) => {
      history.push(`/lms/reports/deleteUsers/company/${cid}`);
    },
    openFetchUsersDialog: (cid) => {
      history.push(`/lms/reports/fetch/company/${cid}`);
    },
    openUpdateUsersStatusDialog: (cid) => {
      history.push(`/lms/reports/updateStatus/company/${cid}`);
    },
    goUsersPage: (cid) => {
      history.push(`/lms/reports/company/${cid}`);
    },
    openBulkInsertDialog: (cid) => {
      history.push(`/lms/reports/bulkinsert/company/${cid}`);
    },
  };
  ...
  return (
    <UsersUIProvider usersUIEvents={usersUIEvents}>
      <UsersLoadingDialog />
      <Switch>
        <Route path="/lms/reports/new/company/:cid">
          {({ history, match }) => (
            <>
              <UserEditDialog
                show={match != null && match.params.cid}
                onHide={() => {
                  history.push("/lms/reports/company/" + match.params.cid);
                }}
              />
        ...
        <Route path="/lms/reports/:id/edit/company/:cid">
          {({ history, match }) => (
            <>
              <UserEditDialog
                show={match != null}
        ...
        <Route path="/lms/reports/:id/detail/company/:cid">
          {({ history, match }) => (
            <>
              <UsersDetailDialog
                show={match != null}
                id={match && match.params.id}
                onHide={() => {
                  history.push("/lms/user-reports");
                }}
              />
            </>
          )}
        </Route>
        <Route path="/lms/reports/deleteUsers">
          {({ history, match }) => (
            <UsersDeleteDialog
              show={match != null}
              onHide={() => {
                history.push("/lms/reports");
              }}
            />
          )}
        </Route>
        <Route path="/lms/reports/:id/delete/company/:cid">
          {({ history, match }) => (
            <>
              <UserDeleteDialog
                show={match != null}
        ...
        <Route path="/lms/reports/company/:cid">
          {({ history, match }) => (
            <>
              <UsersCard companyid={match.params.cid} />
            </>
        ...
        <Route path="/lms/reports/bulkinsert/company/:cid">
          {({ history, match }) => (
            <>
