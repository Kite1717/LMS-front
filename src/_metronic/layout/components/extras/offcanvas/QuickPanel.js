...
export function QuickPanel() {
  ...
  const getCard = (value) => {
    const themes = ["warning", "success", "danger", "primary"]
    let ch;
    do {
      ch = themes[Math.floor(Math.random() * themes.length)]
    } while (ch === prev);

    prev = ch;

    return (
      <div className={`d-flex align-items-center bg-light-${ch} rounded p-5 mb-5`}>
        <span className={`svg-icon svg-icon-${ch} mr-5`}>
          <SVG
        ...
        <div className="d-flex flex-column flex-grow-1 mr-2">
          <a
            href="#"
            className={`font-weight-normal text-dark-75 text-hover-${ch} font-size-lg mb-1`}
          >
            {value.FullName}
          </a>
          <span className="text-muted font-size-sm">
            {value.Detail + "   " + value.IpAddress}
          </span>
        </div>

        <span className={`font-weight-bolder text-${ch} py-1 font-size-lg`}>
        ...
        <div className="offcanvas-content px-10">
          <div className="tab-content">
            ...
            >
              ...
              <div className="mb-5">
                <h5 className="font-weight-bold mb-5">Bildirimler</h5>
